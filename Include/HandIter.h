...
class CHandIterator : public iterator<input_iterator_tag, unsigned long long >
{
private:
	...
	int m_card[MaxNumberOfCards];

	int cardSkip;
private:

	unsigned long long RecalculateMask(int fromCard = 1);
	int& DecrementSkipCounter(int& card);
public:
	CHandIterator(int numberOfCards, unsigned long long shared = 0ULL, unsigned long long dead = 0ULL);
	CHandIterator(const CHandIterator & it);
	CHandIterator& operator++();
	CHandIterator operator++(int) { CHandIterator tmp(*this); operator++(); return tmp; };
	bool operator==(const CHandIterator& rhs) { return m_mask == rhs.m_mask; }
	bool operator!=(const CHandIterator& rhs) { return m_mask != rhs.m_mask; }
	unsigned long long& operator*() { return m_mask; };
	CHandIterator& begin();
	CHandIterator& end();
...
public:		// Lookup Tables
	...
	/// <summary> 1326 ulong cards masks for all hold cards. </summary>
	static const unsigned long long TwoCardTable[];
